...
SOLUTION_EXE_PATH = SOLUTIONS_DIR / "freefan_solution"


def generate_input(test_case_number):
    """
    ฟังก์ชันสำหรับสร้างข้อมูล input สำหรับโจทย์ "แฟนให้ฟรี (Free Fan)"
    """
    # Case 0: Sample 1 from PDF (N=3)
    if test_case_number == 0:
        N = 3
    # Case 1: Sample 2 from PDF (N=4)
    elif test_case_number == 1:
        N = 4
    # Other cases for N from 1 to 16
    # Map test_case_number 2 -> N=1, 3 -> N=2, ..., 17 -> N=16
    elif 2 <= test_case_number <= 17:
        N = test_case_number - 1
    # Default/max case
    else:
        N = 16
